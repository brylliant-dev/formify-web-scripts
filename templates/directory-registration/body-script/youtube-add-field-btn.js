$('.agency_content_main_contact').on('click', function(){
  ...
  $('.popup-close, .contact-popup_bg').on('click', function(){
  ...
  document.addEventListener("DOMContentLoaded", function () {
    /**
     * Updates the displayed count of case studies for each item.
     */
    function updateCaseStudyCounts() {
      const containers = document.querySelectorAll('.agency_desc_case_collection');
  
      containers.forEach((container) => {
        const caseCountDiv = container.querySelector('.agency_desc_case_list');
        let count = 0;
  
        // Count the `.w-dyn-item` elements
        if (caseCountDiv) {
          count = caseCountDiv.querySelectorAll('.w-dyn-item').length;
        }
  
        // Find the `.case-study-count` inside `.agency_content_id` and update it
        const countDisplay = container.querySelector('.agency_content_id .case-study-count');
        if (countDisplay) {
          countDisplay.textContent = `No. of Case Studies: ${count}`;
        }
      });
    }
    ...
    function initialize() {
      ...
      updateCaseStudyCounts();
    }
  
    // Wait for Webflow content to load, then initialize
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    videoLinks.forEach((link) => {
      ...
      const videoParams = getURLParams(videoUrl);
      const videoId = videoParams?.v;
